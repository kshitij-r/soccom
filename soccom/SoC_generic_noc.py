import os
import extract_IO_noc
import interfaceIP_NA_noc
import json


def soc_generator_noc(base_dir, in_json_file):
    ...
    print('NOC')
    soc_connections = noc_dict.copy()
    # print('--------------------------------------------------')
    # print('----------------------------------------------------------')
    # print(soc_connections)

    # Check for existence of Bus based subsystems and if so, 
    # make necassary connections and instantiations
    soc_subsystem = False
    # print(noc_config)
    for outer_key,outer_value in noc_config.items():
        for inner_key,inner_value in outer_value.items():
            #print(noc_config[outer_key][inner_key])        
            if(inner_key == 'bus_type'):
                soc_subsystem = True
    # print(soc_subsystem)
    #########################################################################
    ...
    router = extract_IO_noc.ExtractRtl(base_dir, router_fname)
    ...
    ip = interfaceIP_NA_noc.interface_ip_na(base_dir, ip)
    compute_tile = extract_IO_noc.ExtractRtl(base_dir, ip)
    ...
    param_channels = 2
    param_flit_width = 32
    generated_soc_filename = 'system_noc.sv'
    generated_soc_top_module_name = 'system_noc'
    with open(os.path.join(base_dir, generated_soc_filename), 'w') as file:
        for includes in total_includes:
            file.write(f'`include "dbg_config.vh"\n')
        ...
        file.write('output[4 * 32 - 1:0] wb_ext_adr_i,\n')
        file.write('output[4 * 1 - 1:0] wb_ext_cyc_i,\n')
        file.write('output[4 * 32 - 1:0] wb_ext_dat_i,\n')
        file.write('output[4 * 4 - 1:0] wb_ext_sel_i,\n')
        file.write('output[4 * 1 - 1:0] wb_ext_stb_i,\n')
        file.write('output[4 * 1 - 1:0] wb_ext_we_i,\n')
        file.write('output[4 * 1 - 1:0] wb_ext_cab_i,\n')
        file.write('output[4 * 3 - 1:0] wb_ext_cti_i,\n')
        file.write('output[4 * 2 - 1:0] wb_ext_bte_i,\n')
        file.write('input[4 * 1 - 1:0] wb_ext_ack_o,\n')
        file.write('input[4 * 1 - 1:0] wb_ext_rty_o,\n')
        file.write('input[4 * 1 - 1:0] wb_ext_err_o,\n')
        file.write('input[4 * 32 - 1:0] wb_ext_dat_o\n')
        file.write(');\n')
        file.write('\n\n')
        ...
        file.write('parameter config_t CONFIG = \'x; \n')
        file.write('localparam FLIT_WIDTH = CONFIG.NOC_FLIT_WIDTH; \n')
        file.write('localparam CHANNELS = CONFIG.NOC_CHANNELS; \n')
       ...
       # with open(extract_IO_noc.os.path.join(base_dir, "optimsoc_config_call.sv")) as file2:
            ...
            if connections['is_Router']:
                ...
                for neighbors in connections['neighbors']:
                    if not soc_connections[neighbors]['is_Router']:
                        ip_na_connection = interfaceIP_NA_noc.interface_ip_na\
                            (base_dir, soc_connections[neighbors]['top_module_fname'])
                        ip_na_obj = extract_IO_noc.ExtractRtl(base_dir, ip_na_connection)
                ...
                r_obj = extract_IO_noc.ExtractRtl(base_dir, connections['top_module_fname'])
        ...
        # Initilialize the soc subsystem 
        file.write('')
        file.write('#(.CONFIG(CONFIG),\n')
        file.write('.ID(1),\n')
        file.write('.COREBASE(1*CONFIG.CORES_PER_TILE),\n')
...
#     soc_generator_noc('D:\\Code\\SoC Compiler Tool','hybrid_wb.json')
