...
class ExtractRtl:
    ...
    def file_read(self): #Read whatever .sv or .v file is given and tries to find out what the top module is
    #Assumption: If the file has multiple modules, the first module is usually the 
    #            top module. That is usually how modules are written except for RISC V, 
    #            where the top module is somewhere down. This function removes comments as well.
    ...
    def extract_headers(self):#Takes care of headers like packages and includes.
    ...
    def extract_param(self): #takes care of parameters which are declared either is SV or V
        contents = self.file_read()
        try:
    ...
    def extract_io(self): #This takes care of IO ports
        contents = self.file_read()
...
def main():
    obj = ExtractRtl('/Users/kshitijraj/Study /SoC Compiler Tool', 'networkadapter_conf.sv')
    obj.extract_io()
    obj.extract_module()
    obj.extract_param()
